  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [cRes, mRes, pRes] = await Promise.all([
        ...
        supabase.from("majors").select("*").eq("college_id", id).order("created_at", { ascending: false }),
        supabase.from("professors").select("*").eq("college_id", id).order("created_at", { ascending: false }),
      ]);
      ...
      <div className="card p-6 md:p-8">
        {loading ? (
          <div className="animate-pulse">
            <div className="h-4 w-28 bg-brand-100 rounded" />
          ...
          <div className="card p-6">
            ...
            <div className="mt-4 grid gap-3">
              {majors.length === 0 ? (
                ...
                majors.map((m) => (
                  <div key={m.id} className="rounded-xl border border-brand-100 p-4 hover:bg-brand-50 transition">
          ...
          <div className="card p-6">
            ...
            <div className="mt-4 grid gap-3">
              {professors.length === 0 ? (
                ...
                professors.map((p) => (
                  <div key={p.id} className="rounded-xl border border-brand-100 p-4 hover:bg-brand-50 transition">
                    ...
                    {p.bio && (
                      <div className="mt-2 text-sm text-slate-600">{p.bio}</div>
                    )}
                  </div>
                ))
              )}
